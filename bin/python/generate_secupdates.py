...
# This script just searches the opensuse-secupdates mailinglist for posts written max 7 days ago. Then it transforms it to the Wiki Layout for using in the openSUSE Weekly News.
...
URL = "http://lists.opensuse.org/opensuse-security-announce/mailinglist.rss"
...
#            print (int(thismonth)-int(mymonth))
            if not thismonth == mymonth:
                ...
                diff = int(myday)-int(thisday)
                if diff == i:
                    articles[myindex]=[data[k][0], data[k][1], strip_ml_tags(data[k][2]), k]
    myindex = myindex+1
#print data[k][2]
#print articles
...
for k in iterate:
 ...
 print "====["+str(url)+" "+str(str(subj).split("[security-announce]")[1].strip())+"]===="
 #print "#####################"
 y = []
